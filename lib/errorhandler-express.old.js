...
module.exports = {
  ...
  errorHandling(options) {
    ...
    let errors = lodash.merge({
      ...
      translator = options.translate || function (a) { return a; },

      parsers = lodash.merge({
        mongoose(err) {
          ...
          if (err.info.detail instanceof mongoose.Error.ValidationError) {
            ...
            lodash.each(err.info.detail.errors, (error, index) => {
    ...
    return function (err, req, res, next) {
      if (err instanceof kError) {
        ...
        err.info = lodash.merge(ret, err.info);


                    // execute each parser
        lodash.each(parsers, (parser) => {
          parser(err);
        });

        err.info.msg = translator(err.info.msg);
        ...
        res.status(500).json({
          ...
          msg: translator(errors.default.msg),
          detail: err.stack,
        });
      } else {
        next();
